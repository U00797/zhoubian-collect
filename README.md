...
小红书/微博官方周边宣传页的一键识别归档工具。浏览器插件采集正文和图片后交给本地桥接服务。默认走 AI 识别和归档；没有 AI 时会生成可直接打开、编辑和导入的本地结果包。

## 安装

1. 在 Edge/Chrome 打开 `edge://extensions` 或 `chrome://extensions`，开启“开发人员模式”，选择“加载已解压的扩展程序”，选中本目录。
2. 双击 `start-server.cmd` 启动本地桥接服务，窗口显示 `Zhoubian bridge ready`。
3. 打开小红书或微博宣传页，选择处理方式后点击按钮。
