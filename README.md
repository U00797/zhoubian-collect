...
1. 双击 `start-server.cmd` 启动本地桥接服务，窗口显示 `Zhoubian bridge ready`。缺少图片处理或 Excel 导出依赖时脚本会自动安装。
...
网站模式直接填写网址；关键词可以不填，多个关键词用逗号分隔。生成结果会同时读取正文和商品图片，包含出品方、发售时间及地点、系列、IP名称、周边明细、尺寸丨材质丨工艺、价格、图片和备注，并可导出 Markdown、单文件 HTML、ZIP、ENEX、CSV 或带嵌入图片的 Excel（XLSX）。
